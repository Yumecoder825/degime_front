...
export default function Fourimagelink({FileUpload, isClear, onChangeData}) {
  ...
  const handleChildData = (childData) => {
    ...
    setChildDataArray((prevChildDataArray) => {
      ...
      onChangeData({data:updatedChildDataArray, type:"fourImage"});
  ...
  return (
    <div className="flex gap-2 justify-between">
      ...
      <div><Imagelinkitem FileUpload={FileUpload} isClear={isClear} id = {3} onChangeState={handleChildData} /></div>
    </div>
  )
}
