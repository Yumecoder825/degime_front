...
import { myDelete, myGet, myPut } from '../../utilities/config';
...
export default function Chatlist() {
  ...
  const startChat = (index) => {
    // console.log(index);
    localStorage.setItem('chatroom', chatGroup[index].room_name);
  ...
  const handleDelete = async () => {
    ...
    for(const index in selectedList){
      let response = await myDelete(`social/private/chatroom?room_name=${chatGroup[index].room_name}`, header);
      console.log(response);
    }
    let appliedData = await myGet('social/private/chatroom', header); 
    if (appliedData){
  ...
  const handleCreate = async () => {
    ...
    if(memberList.length < 2) toast.error("Members must be large than 2");
      ...
      let res = await myPut('social/private/chatroom?new_room=True', {chat_group:groupName,member:memberList}, header);
      console.log("Res: ", res);
  ...
  useEffect(()=>{
    async function dispatch(){
      if(isActive) {
        ...
        let incomeData = await myGet('social/private/contactdata?is_incoming=True', header);
    ...
    <div>
      ...
      <div className="w-[80%] mx-auto relative">
        ...
        }
        {
          ...
          <div className='absolute -right-5 top-24 close rounded-md cursor-pointer hover:opacity-60 active:opacity-90' onClick={()=>{setIsCreate(false); setIsDeleteState(false)}}>
        ...
        <div className='flex gap-x-2'>
          {
            isCreate && (
              selectedList.map((item, index)=>(
                <img key = {index} alt="selected user" src = {chatGroup[item].member.length > 1 ? '/image/focus-group.png': chatGroup[item].member[0].avatar || '/image/user_default.png'} width={50} height={50} className='rounded-full' />
              ))
            )
          }
        </div>
