    ...
    return (
        <div className='px-10 mb-3'>
            <div className="flex justify-between items-center mb-2">
                <button className="bg-indigo-700 opacity-80 hover:opacity-90 active:opacity-50 text-white py-2 max-md:px-1 md:px-5 rounded-full text-center">{moment().format("YYYY/MM/DD")} 発送済み</button>
            </div>
            <TableContainer component={Paper}>
                <Table sx={{minWidth: 700}} aria-label="customized table">
                    ...
                    <TableBody>
                        {
                            shipped.length === 0 && <StyledTableRow><StyledTableCell colSpan={15} align="center">該当する注文は 0 件 です</StyledTableCell></StyledTableRow>
                        }
                        {shipped.map((p, index) => (
