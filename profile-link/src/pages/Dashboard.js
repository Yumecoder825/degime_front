...
// import Dashboardbadge from "../components/Dashboardbadge.js";
...
const Dashboard = (props) => {
  ...
  const handleVerifyEmail = async () => {
    try {
      const response = await axios.post(
        `${myConfig.apiUrl}/account/forgot_password`,
        {email:verifyEmail}
      );
  ...
  const handleResetPassword = async () =>{
    try {
      const response = await axios.post(
        `${myConfig.apiUrl}/account/change_password`,
  ...
  const handleCopy = () => {
    navigator.clipboard
      ...
      .then(() => {
        console.log("Value copied to clipboard:", currentUser.email);
      })
      .catch((error) => {
  ...
  const postRegister = async () => {
    if (isLogin) {
      ...
      if (verifyPassword !== registerData.password) {
        ...
        try {
          const response = await axios.post(
            `${myConfig.apiUrl}/account/register/vcode`,
            registerData
          );
  ...
  return (
    <div className="Dashboard">
      <div className="selectiveBackground flex justify-between">
        <Link to="/dashboard">
      ...
      <div className="mainUserInfo">
        ...
        <div className="userInformation">
          ...
          <div className="flex items-center justify-center ml-3">
            <div className="userEmail">{currentUser.email === "" ? <Link onClick={()=>setShowModal(true)} className="hover:underline hover:text-sky-400 active:text-sky-700" >ログインするにはここに</Link> : currentUser.email}</div>
            <div
              onClick={handleCopy}
              className="w-[20px] inline-block cursor-pointer ml-2"
            >
              <img alt="copy" src="/image/copy.png" className={currentUser.email === "" ? "hidden" : "inline-block" } />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mainButtons">
        <span className="searchButton button">
          ...
          <form autoComplete="off">
            ...
            <input type="text" role="presentation" autoComplete="off" className="searchInput" id="searchinput" placeholder="検索" />
          </form>
          <div id="deleteIcon">
            <svg
              width="22"
      ...
      <div className="bottom">
        <Link to="/chat/list" className="chat bottomLink">
        ...
        {isAdmin && (
          <Link to="/admin" className="top bottomLink">
            <svg
              fill="#a855f7"
              className="bottomIcon"
              version="1.1"
                ...
                <form className="loginMainInfo mt-[30px]" action="">
                  ...
                  {isLogin && (
                    <Link
                      ...
                      onClick={()=>{setShowModal(false); setShowForgottenModal(true)}}
                    >
                      パスワードを忘れた方
                    </Link>
                  )}
        ...
        >
          <TEModalContent>
            ...
            <TEModalBody>
              {
                isEnteredEmail ?
                <NumberInputComponent email={verifyEmail} isForgotten={showForgottenModal} isSuccessful={handleShowFogorttenModal} />
                : 
                <>
                  <div className='w-full flex justify-center flex-col'>
                    <label htmlFor="verifyEmail">メールアドレス</label>
                    <input value={verifyEmail} onChange={(e)=>setVerifyEmail(e.target.value)} type="email" id="verifyEmail" className="border p-2"/>
                  </div>
                  <div className='flex justify-center mt-10 mb-5' >
                    <TERipple>
                      <button type="button" className='px-5 py-2 rounded-full bg-green-500 cursor-pointer text-white' onClick={handleVerifyEmail} >確 認</button>
                    </TERipple>
                  </div>
                </>
              }
          ...
          <TEModalContent>
            ...
            <TEModalBody>
              <div className='w-full flex justify-center flex-col'>
                ...
                <input value={oldPassword} onChange={(e)=>setOldPassword(e.target.value)} type="password" id="oldPassword" className="border p-2"/>
              </div>
              <div className='w-full flex justify-center flex-col'>
                <label htmlFor="newPassword">新しいパスワード</label>
                <input value={newPassword} onChange={(e)=>setNewPassword(e.target.value)} type="password" id="newPassword" className="border p-2"/>
              </div>
              <div className='flex justify-center mt-10 mb-5' >
                <TERipple>
                  <button type="button" className='px-5 py-2 rounded-full bg-green-500 cursor-pointer text-white' onClick={handleResetPassword} >確 認</button>
                </TERipple>
              </div>
            </TEModalBody>
          </TEModalContent>
